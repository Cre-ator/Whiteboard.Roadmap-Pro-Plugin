...
class roadmap
{
    ...
    /**
     * returns true, if all done bugs are in status closed
     *
     * @return bool
     */
    private function checkAllBugsAreClosed()
    {
        $allBugsAreClosed = TRUE;
        $doneBugIds = $this->getDoneBugIds();

        foreach ($doneBugIds as $doneBugId) {
            if (bug_get_field($doneBugId, 'status') != CLOSED) {
                $allBugsAreClosed = FALSE;
            }
        }

        return $allBugsAreClosed;
    }

    /**
     * returns the latest changed-to-closed-date from $this->doneBugIds
     *
     * @return null
     */
    private function getLatestSetToClosedDateBugList()
    {
        $latestSetToClosedDate = 0;
        $doneBugIds = $this->getDoneBugIds();

        foreach ($doneBugIds as $doneBugId) {
            $setToClosedTimeStamp = $this->getLatestSetToClosedDateBug($doneBugId);
            if ($setToClosedTimeStamp > $latestSetToClosedDate) {
                $latestSetToClosedDate = $setToClosedTimeStamp;
            }
        }

        return $latestSetToClosedDate;
    }

    /**
     * returns the latest changed-to-closed-date from $bugId
     *
     * @param $bugId
     * @return int
     */
    private function getLatestSetToClosedDateBug($bugId)
    {
        $latestSetToClosedTimeStamp = 0;

        if (is_numeric($bugId) && ($bugId != NULL)) {
            $mysqli = rProApi::initializeDbConnection();

            $query = /** @lang sql */
                'SELECT MAX(date_modified) AS date_modified FROM mantis_bug_history_table ' .
            ...
            $latestSetToClosedTimeStamp = $dbResultRow[0];
        }

        return $latestSetToClosedTimeStamp;
    }
    ...
    {
        if ($this->checkAllBugsAreClosed()) {
            $this->expectedFinishedDate = $this->getLatestSetToClosedDateBugList();
        } else {
